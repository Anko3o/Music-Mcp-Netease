...
- 🎧 **点歌 MCP（十二把工具）** — 你的 AI 伴侣可以搜歌、读歌词、发歌曲卡/歌词卡、把歌插进你的播放队列、翻批注本、看你最近在听什么、刷评论区、往共享歌单里收歌 → 见 [`mcp/`](./mcp/)
...
| `lyric_read` | 读整篇歌词（时间戳＋原文｜译文），也可按区间只读一段（from_seconds / to_seconds，秒或 m:ss），挑句前先看一遍；正主没词时借同专辑同名那条的词并注明出处 |
...
> 更新 · 2026-09-26（MCP v1.4.0）：新增读歌词小工具 `lyric_read`，可整篇或按区间。
