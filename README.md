...
> \* 说明：MV 悬浮小窗是系统级画中画（本仓库自带，盖在哪个 App 上都行）；图里底下那层聊天页和顶栏的「一起听」进度药丸是我们自己家的聊天前端，**不随本仓库发布**——但播放器已备好全套对接件（iframe 嵌入、`music:*` postMessage 协议、`CARD_WEBHOOK` 收卡接口），接上你家的聊天系统就能长成这样。
